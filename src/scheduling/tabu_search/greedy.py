...
from src.scheduling.tabu_search import greedy_tool
import math
import time
...
def greedy_algorithm(network, QUEUE_SIZE=10000):
    """
    贪婪算法主函数
    ...
    for i in range(len(flow_name)):
        print("------------------------------------------第{0}条流--------------------------------".format(flow_name[i]))
        # 流信息
        print("flow deadline", flow_deadline[i])
        print("flow period", flow_period[i])
        ...
        for path_index in range(len(flow_path_id[i])):
            ...
            for current_slot in range(maxoffset, pktslot-1, -1):
                ...
                if flag:
                    ...
                    fail = 0
                    print("the flow is {0}, and the choose slot is {1}".format(flow_name[i], f'slot{current_slot}'))
                    print("the route we choose is {}".format(network.flow_path[i][path_index]))
                    ...
                    print("the flow e2e delay is", greedy_tool.e2e_delay(through_slot, slot, current_slot, all_e2e) - flow_starttime[i])
                    break
                else:
                    continue
        ...
        if fail:
            ...
            print("can't schedule", flow_name[i])
    ...
    print("----------------------------------------调度结束-----------------------------------")
    print("成功调度流量数目{0}".format(len(schedule_success)))
    print("失败调度流量数目{0},流量包括:{1}".format(len(schedule_fail), schedule_fail))
    print("调度成功率为：", 1 - len(schedule_fail) / len(flow_name))

    print('调度后网络', queue_list)
    # 保存调度矩阵与队列矩阵
    columns = [f"slot{i}" for i in range(slotnum)]
    idx = list(network.net_edge_port_dict)
    greedy_tool.list_to_excel(schedule_list, queue_list, flow_list, columns, idx)

    # print(np.array(queue_list))
    per_q = []
    for i, j in enumerate(actions_slot_route):
        if flow_name[i] in schedule_success:
            j = j % slotnum
            per_q.append(j)
        else:
            per_q.append(-1)
