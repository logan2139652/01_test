...
class Maze(object):
    ...
    def __init__(self, net, queue_size=None):
        ...
        self.S = np.concatenate((np.array([0]), np.array(self.route_feature[0][0]),
                           np.array([self.PERIOD[0]]), np.array([self.DEADLINE[0]])))
        self.R_sum = 0  # 初始奖励总和
    ...
    def step(self, action):
        ...
        S_NEW = np.concatenate((np.array([flow+1]), np.array(self.route_feature[flow+1][0]),
                           np.array([self.PERIOD[flow+1]]), np.array([self.DEADLINE[flow+1]])))
