...
class DQN(object):
    def __init__(self):  # 定义DQN的一系列属性
        ...
        self.greedy_increment = 0

    def choose_action(self, x):  # 定义动作选择函数 (x为状态)
        x = torch.unsqueeze(torch.FloatTensor(x), 0)  # 将x转换成32-bit floating point形式，并在dim=0增加维数为1的维度
        x = x.cuda()
        # if self.learn_step_counter % TARGET_REPLACE_ITER == 0:  # 一开始触发，然后每100步触发
        #     self.greedy_increment += 0.001
        if np.random.uniform() < EPSILON+self.greedy_increment:  # 生成一个在[0, 1)内的随机数，如果小于EPSILON，选择最优动作
            actions_value = self.eval_net.forward(x)  # 通过对评估网络输入状态x，前向传播获得动作值
            actions_value = actions_value.cpu()
            # print("1", actions_value)
...
reward_plt = []
sched_plt = []
for i in range(max_iteration):  # 400个episode循环
    print('<<<<<<<<<Episode: %s' % i)
    s = env.reset()  # 重置环境
    ...
    while True:  # 开始一个episode (每一个循环代表一步)
        ...
        if done:  # 如果done为True
            ...
            reward_plt.append(episode_reward_sum)
            sched_plt.append(len(env.has_schedule))
            break  # 该episode结束
...
torch.save(dqn.target_net.state_dict(), 'dqn_params.pth')