...
@ author:hx
@ data:2023/05/09
mininet数据处理 与 src.scheduling.data_processing.mininet_data.py和算法模块同用
...
import math
import numpy as np
from src.db.topology.MiniNETtopo import mininetTopo, topoRoute
...
def hopSlotGet(flow_path, long_edge):
    """
    计算端到端时隙
...
class MiniNet:
    def __init__(self, flow_number, excel_path=None, test_slot_size=None, CORE_NODE=None):
        """
        初始化网络
        :param path:流量文件存放路径
        ...
        if excel_path is None:
            excel_path = r'D:\Python\PyCharmWorkplace\01ArticleOneCode\src\db\dataset\flow_mininet_1000.xlsx'

        # .xlsx文件读取
        DATA_PATH = excel_path
        # df = pd.read_excel(DATA_PATH)[:20]
        ...
        flow_hop_slot = hopSlotGet(flow_path, long_edge)
...
if __name__ == '__main__':
    mininet = MiniNet(excel_path=r'D:\Python\PyCharmWorkplace\01ArticleOneCode\src\db\dataset\flow_mininet_1000.xlsx',
                      flow_number=1000, test_slot_size=50)
    print(mininet.flow_path_id[999])
