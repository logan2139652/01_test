...
if __name__ == '__main__':
    ...
    actions, actions_slot_route = greedy_algorithm(net, QUEUE_SIZE=queue_capacity)
    print(actions_slot_route)
    ...
    for i in range(1):
        ...
        RL = QLearningTable(n_states=env.n_states, actions=env.action_space,
                            learning_rate=0.1, reward_decay=0.9, e_greedy=0.9)
        best_result, schedule_plt = update(env, RL, times=1)
        print('第{}次结果为{}'.format(i+1, len(best_result[1])))
        # 在通过q-learning指导greedy
        actions, actions_slot_route = greedy_algorithm(net, best_result, QUEUE_SIZE=queue_capacity)
